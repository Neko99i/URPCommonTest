...
namespace LWGUI
{
	...
	/// group：father group name, support suffix keyword for conditional display (Default: none)
	/// keyword:有贴图时开启keyword
	/// style:0 绘制小图 ;1 绘制小图加 +ST ;2 大图 + ST
	...
	internal class TexDrawer : SubDrawer
	{
		...
		public TexDrawer() : this("", "_")
		{
		}

		public TexDrawer(string group) : this(group, "_")
		{
		}

		public TexDrawer(string group, string keyword) : this(group, keyword, "_")
		{
		}

		public TexDrawer(string group, string keyword, string extraPropName) : this(group, keyword, extraPropName, 0)
		{
		}

		public TexDrawer(string group, string keyword, string extraPropName, float style)
		{
			this.group = group;
			this._keyword = keyword;
        ...
        public override void DrawProp(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
        {
            ...
            switch (_style)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    editor.TextureProperty(prop, label.text, true);
                    break;
            }

            bool texEnabled = prop.textureValue != null;

            if (EditorGUI.EndChangeCheck())
            {
                if (_keyword != "_" && _keyword != "")
                    Helper.SetShaderKeyWord(editor.targets, Helper.GetKeyWord(_keyword, prop.name), texEnabled);
            }
            //当添加了贴图时会自动开启Keyword
            ...
            if (extraProp != null && extraProp.type != MaterialProperty.PropType.Texture)
            {
                ...
                switch (extraProp.type)
                {
                    ...
                    case MaterialProperty.PropType.Vector:
                        ...
                        extraPropRect = MaterialEditor.GetRectAfterLabelWidth(indentedRect);
                ...
                if (extraProp.type == MaterialProperty.PropType.Vector)
                    ...
                    editor.ShaderProperty(extraPropRect, extraProp, string.Empty);
