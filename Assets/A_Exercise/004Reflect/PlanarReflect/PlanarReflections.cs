...
#if UNITY_EDITOR
            width = (int) UnityEditor.Handles.GetMainGameViewSize().x; //Screen.currentResolution.width;
            height = (int) UnityEditor.Handles.GetMainGameViewSize().y; //Screen.currentResolution.height;
#endif
            var x = (int)(width * scale * GetScaleValue());
            var y = (int)(height * scale * GetScaleValue());
            return new int2(x, y);
        }
